...
const BracketTree = ({ data, mode = 'results' }) => {
  ...
  const U = compact ? 34 : 40;         // R32 row height
  const CARD_W = compact ? 86 : 104;   // card / column width
  const GAP = compact ? 14 : 20;       // gap between columns (connectors live here)
  const CARD_H = compact ? 24 : 26;
  const scrollRef = useRef(null);

  // Center the horizontal scroll so the Final is in view when the bracket overflows
  // (typical on phones). No-op when it already fits.
  useEffect(() => {
  ...
  const H = U * 8;
  const nodeProps = (key) => (mode !== 'path' ? { highlight: null, dim: false } : { highlight: pathSet.has(key), dim: !pathSet.has(key) });
  ...
  const Card = ({ item, highlight, dim, isFinal }) => {
    ...
    const indicator = highlight != null
      ? (highlight ? `+${ROUND_PTS[item?.round] ?? ROUND_PTS.F}` : '')
      : status === 'won' ? '✓' : status === 'out' ? '✗' : '';
    return (
      <div style={{ height: CARD_H }} className={`relative z-10 rounded border px-1.5 flex items-center gap-1 leading-none
        ${compact ? 'text-[10px]' : 'text-[11px]'} ${STATUS_STYLE[status]} ${champion ? 'ring-1 ring-wc-accent' : ''}
        ${highlight ? 'ring-2 ring-wc-accent shadow-glow !text-white !bg-wc-accent/20' : ''} ${dim ? 'opacity-25' : ''}`}>
        ...
        {indicator && <span className="shrink-0 text-[9px] opacity-90">{indicator}</span>}
      </div>
    );
  };
  ...
  const label = (round, i) => (
    <div key={i} style={{ width: CARD_W }} className="text-center text-[9px] uppercase tracking-wide text-gray-500 shrink-0">
  ...
  return (
    <div>
      <div ref={scrollRef} className="overflow-x-auto -mx-1 px-1 pb-1" style={{ WebkitOverflowScrolling: 'touch' }}>
        <div className="min-w-max">
          <div className="flex mb-1" style={{ gap: GAP }}>
            {['R32', 'R16', 'QF', 'SF', 'F', 'SF', 'QF', 'R16', 'R32'].map((r, i) => label(r, i))}
          </div>
      ...
      <p className="sm:hidden text-center text-[10px] text-gray-500 mt-1">← swipe to explore the full bracket →</p>
    </div>
  );
};
